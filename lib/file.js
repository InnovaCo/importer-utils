...
function globAll(patterns, options, callback) {
...
module.exports = {
	...
	normalize: function(fileObj) {
		...
		if (typeof fileObj === 'object' && !Buffer.isBuffer(fileObj)) {
			...
			var patters = Array.isArray(fileObj.src) ? fileObj.src : [fileObj.src];
			options.ignore = options.ignore || [];
			patters = patters.filter(function(p) {
				if (p[0] === '!') {
					options.ignore.push(p.substr(1));
				} else {
					return true;
				}
			});
	...
	read: function(fileObj, callback) {
		...
		async.waterfall([
			function(callback) {
				globAll(files, merge(true, fileObj.options, {mark: true}), callback);
			},
